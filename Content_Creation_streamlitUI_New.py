...
def processed_text(doc, Topic, file):
    ...
    unwanted_words = ["course", "courses", "certification", "certifications", "professional", "professionals", "salary", "salaries", "week", "weeks"]
...
#            print(len(ind))
    ...
    #                                st.write(para.text) 
        ...
        for k in lkk1:
            st.write(summarizer.summarize(k, ratio=0.5))
#        st.write("Raw Text:", "\n\n".join(lkk1))
#        st.write("Summarized Text:", summarizer.summarize("\n\n".join(lkk1), ratio=0.4))
...
#            st.dataframe(df2_filter['URLs'].style.highlight_max(axis=0))
#            st.write(df3['URLRank_URLS'])
#            html = df3.style.set_table_attributes('class="table-style"')
#            st.write(html)
#            st.dataframe(df3.style.set_properties(subset=['URLs'], **{'width': '800px'}))
#            st.markdown(df3['URLRank_URLS'])
#            st.table(df3.to_string(justify='left', col_space='30', index=False))
#            st.dataframe(pd.DataFrame(df2_filter['URLs'], ignore_index = True).style.set_properties(subset=['URLs'], **{'width': '800px'}))
...
#            clean_links = Extract_Ranked_urls(links)
            ...
            processed_text(doc, Topic, "Model Output - " + str(Topic)+".docx")
...
#            st.write(heads)
    ...
    st.sidebar.markdown("*******************************")
    if st.sidebar.button("Download Contents as PDF"):
        with st.spinner("Downloading..."):
#            wdFormatPDF = 17 #17 = PDF format
#
#
#            # absolute path is needed
#            # be careful about the slash '\', use '\\' or '/' or raw string r"..."
#            in_file= 'C:\\Users\\Darcey\\Downloads\\Model Output - ' + str(Topic)+'.docx'
#            out_file= 'C:\\Users\\Darcey\\Downloads\\Model Output - ' + str(Topic)+'.pdf'
#
#            # create COM object
#            word = comtypes.client.CreateObject('Word.Application')
...
#            word.Quit() # close Word Application
            convertFiletoPDF("Model Output - " + str(Topic)+".docx")

#    st.sidebar.markdown("*******************************")
#    if st.sidebar.button("View Processed Contents"):
#        processed_text(Topic, "Model Output - " + str(Topic)+".docx")
